...
if ! [[ "${CYCLES}" =~ ^[0-9]+$ ]] || [[ "${CYCLES}" -le 0 ]]; then
  ...
  echo "usage: ./scripts/cli_relogin_smoke.sh [cycles]" >&2
  exit 2
fi
...
assert_key_works() {
  local key="$1"
  if [[ -z "${key}" ]]; then
    echo "config api_key is empty after recovery" >&2
    exit 7
  fi
  if [[ "${key}" == "${INVALID_KEY}" ]]; then
    echo "config api_key was not refreshed after recovery" >&2
    exit 8
  fi
  if ! [[ "${key}" == nbl_* ]]; then
    echo "config api_key has unexpected prefix after recovery" >&2
    exit 9
  fi
  curl -fsS -H "Authorization: Bearer ${key}" "${API_BASE}/api/keys/" >/dev/null
}

echo "cli relogin smoke: cycles=${CYCLES} api=${API_BASE}"
echo "logs: ${RUN_LOG}"
...
if [[ -z "${username}" ]]; then
  echo "config username missing, run login first" >&2
  exit 10
fi

passes=0
for ((i = 1; i <= CYCLES; i++)); do
  cycle_log="${RUN_LOG}/cycle-${i}.log"
  write_invalid_key
  assert_health
  assert_no_rogue_port_8000

  if ! "${EXPECT_SCRIPT}" "${BINARY}" "${TIMEOUT_SECONDS}" "${cycle_log}"; then
    echo "cycle ${i}: FAIL (see ${cycle_log})" >&2
    exit 11
  fi
...
echo "cli relogin smoke complete: ${passes}/${CYCLES} passed"

...
func TestViewTabBranchesAndRowHighlightExtraMatrix(t *testing.T) {
	app := NewApp(nil, &config.Config{})
	app.width = 100
	app.height = 30
	...
	app.tab = tabLogs
	app.tabNav = false
	app.logs.addValue.Active = true
	assert.True(t, app.rowHighlightEnabled())

	app = NewApp(nil, &config.Config{})
	app.tab = tabFiles
	app.tabNav = false
	app.files.addMeta.Active = true
	assert.True(t, app.rowHighlightEnabled())

	app = NewApp(nil, &config.Config{})
	app.tab = tabProtocols
	app.tabNav = false
	app.protocols.editMeta.Active = true
	assert.True(t, app.rowHighlightEnabled())
